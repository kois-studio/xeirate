...
- The current persisted schema is `schemaVersion: 5` under `xeirate.workspace`, validated with Zod. It contains participants, sessions, session conditions, reusable person conditions, selected recurring weekdays, and the latest generated schedule. Legacy `xeirate.workspace.v1` and schema versions 2–4 are migrated in memory and covered by deterministic tests.
